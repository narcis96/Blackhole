...
#else
#include <windows.h>
#define CLIENT "Client (" << GetCurrentProcessId() << ") : "
#endif
#define MY_DEBUG
...
public:
  ...
  {
    assert(n > 0 && moves > 0);
    m_cellState = std::vector<std::vector<CellState>>(
      m_dimension, std::vector<CellState>(m_dimension, CellState::EMPTY));
    m_cellValue = std::vector<std::vector<int>>(
      m_dimension, std::vector<int>(m_dimension, 0));
  ...
  std::tuple<int, int, int> GetTurn()
  {
    ...
    if (m_turn <= m_startMoves) {
      auto maxPos =
        std::max_element(m_freeCells.begin(), m_freeCells.end(),
...
main(int argc, const char* argv[])
{
  ...
  int stopFinal = -1, startMoves = -1;
  if (argc > 1) {
    bool weightsSetted = false;
    for (int i = 1; i < argc - 1; i++) {
      if (strcmp(argv[i], "-weights") == 0) {
        weightsSetted = true;
        sscanf_s(argv[i + 1], "[%Lf, %Lf, %Lf]", &weights[0], &weights[1],
                 &weights[2]);
      }
  ...
  auto OpponentTurn = [ConvertToIndex, &solver](const std::string& command) {
    char line, pos;
    int val;
    sscanf_s(command.c_str(), "%c%c=%d", &line, &pos, &val);

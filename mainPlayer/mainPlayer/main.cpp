...
public:
	BlackholeSolver(const unsigned long& n, const int& moves, const long double *weights, const unsigned long& stopFinal, const int& startMoves)
		: m_dimension(n)
		, m_turn(0)
		, m_totalMoves(moves)
		, m_codif(0LL)
...
int main(int argc, const char* argv[])
{
	...
	int stopFinal, startMoves;
	if (argc > 1)
	{
		for (int i = 1; i < argc - 1; i++) {
			if (strcmp(argv[i], "-weights") == 0)
			{
				sscanf(argv[i + 1], "%Lf,%Lf,%Lf", weights, weights + 1, weights + 2);
			}
	...
	const int moves = 15;
	BlackholeSolver solver(dimension, moves, weights, stopFinal, startMoves);
